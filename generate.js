  ...
  .map((dir) => {
    const htmlFiles = fs
      ...
      .map((f) => {
        ...
        return {
          name: f.replace(".html", ""),
          path: `${encodeURIComponent(dir)}/${encodeURIComponent(f)}?k=4c129f69f551195cbc5e8b57cc5a0975b6efa1e8510573524d11df03c7d010c6`,
          modified,
        };
      });
...
const sectionsHtml = sections
  .map(
    ({ section, items }) => `
    <div class="card" data-section="${escapeHtml(section)}">
...
<body>
  ...
  <div class="app" id="app">
    ...
    <div class="footer">
      <a href="https://github.com/lukinab/ux" target="_blank" rel="noopener">
      ...
      <a href="https://github.com/lukinab/ux/actions" target="_blank" rel="noopener">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
        Actions
      </a>
    </div>
  </div>
  <script>
    ...
    function runFilter(value) {
      ...
      cards.forEach(function (card) {
        var sectionName = normalize(card.getAttribute('data-section') || '');
        var sectionMatch = q === '' || sectionName.includes(q);

        var items = card.querySelectorAll('a.item');
        var dividers = card.querySelectorAll('.file-divider');
        ...
        if (sectionMatch) {
          ...
          visibleItems.forEach(function (item, idx) {
            if (idx > 0) {
              ...
              if (prev && prev.classList.contains('file-divider')) {
                prev.style.display = '';
              }
            }
          });
    ...
    // Spustí se při načtení i při návratu zpět (bfcache)
    window.addEventListener('pageshow', checkBadges);
